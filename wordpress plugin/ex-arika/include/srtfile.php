 ...
 * MA 02110-1301, USA. 
 *
 * @author Julien 'delphiki' Villetorte <gdelphiki@gmail.com> (original)
 * @author of this version DSR! 
...
class srtFile{
	 ...
	 */
	public function __construct($_filename, $_encoding = '', $_file_content = ''){
		$this->filename = $_filename;
		$this->encoding = $_encoding;
		$this->file_content = $_file_content;
		$this->stats = array(
			'tooSlow' => 0,
			'slowAcceptable' => 0,
			'aBitSlow' => 0,
	...
	private function convertEncoding(){
		//lo modifico para saber la codificacion sin usar exec
		/*$exec = array();
		...
		//http://php.net/manual/es/function.mb-detect-encoding.php
		$this->encoding = mb_detect_encoding($this->file_content, 'auto');
		...
		switch($tmp_encoding){
			...
			break;
			default:
				//con castellano falla la deteccion de utf asi que comento el 2 metodo empleado, que es justamente el que falla!
				//if(strtolower(substr($this->encoding, 0, 3)) == 'utf' && self::detect_utf_encoding($this->file_content)){
				if(strtolower(substr($this->encoding, 0, 3)) == 'utf'){
					$this->has_BOM = true;
				} else if(strtolower(substr($this->encoding, 0, 7)) == 'unknown'){
					$this->encoding = self::default_encoding;
					$this->file_content = self::cp1252_to_utf8($this->file_content);		
				}else{
	...
	private function parseSubtitles(){
		...
		$this->tLines = $entries_count; //para que voy a calcular dos veces lo mismo?
		for($i=0; $i<$entries_count; $i++){
			//cambio la forma en que se cargan, de objeto a array!
			/*$sub = new srtFileEntry($matches[1][$i], $matches[2][$i], $matches[3][$i]);
			$this->subs[] = $sub;*/
			$this->subs[] = Array('startTC' => $matches[1][$i], 'stopTC' => $matches[2][$i], 'text' => $matches[3][$i]);
		}
	}
